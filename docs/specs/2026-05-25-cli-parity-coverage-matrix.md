...
- Release impact: M003 maps all 21 built-in slash commands with palette-first dispositions. Six commands wire real Desktop actions through the palette; the remainder register visible deferral notices or existing UI aliases rather than silent gaps.
...
- Disposition: M003 mapping and palette implementation complete for built-in slash commands (S011–S015); lands via `cursor/m003-palette-integration-cdb1`. The composer `/` palette is the palette-first Desktop affordance; detail in the mapping matrix.
