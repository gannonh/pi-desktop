...
- **Running the Electron desktop app on Linux:** It works on the Cloud VM's XFCE display (`DISPLAY=:1`), but needs these non-obvious steps:
  ...
  5. **Blank window troubleshooting:** If the `pi-desktop` window opens but stays solid black (`#0a0a0a`), it is usually one of two Linux VM issues, not a missing window:
     ...
     - **GPU compositing paints nothing to X11:** Linux builds call `app.disableHardwareAcceleration()` in `src/main/index.ts`. After changing main-process code, restart Electron (`rs` in the `pnpm dev` terminal, or stop and rerun `pnpm dev`) — hot renderer reload is not enough.
...
- **Electron UAT video on Cloud VM (preferred recipe):**
  ...
  2. **Start the app visibly:** In tmux, run `pnpm dev` with `DISPLAY=:1`, `ELECTRON_DISABLE_SANDBOX=1`, and any seeded `PI_DESKTOP_USER_DATA_DIR` / `PI_DESKTOP_SMOKE_PI_SESSION=1` env needed for the scenario.
  ...
  4. **Record:** Use `RecordScreen` (or `ffmpeg -f x11grab -video_size 1920x1200 -i :1`) while driving the flow. For scripted terminal/session UAT, headed Playwright is reliable once the app paints: `PI_DESKTOP_SMOKE_HEADED=1 ELECTRON_DISABLE_SANDBOX=1 pnpm exec playwright test tests/smoke/m07-terminal-uat.spec.ts` (set `UAT_EVIDENCE_DIR` for screenshots). Start recording **before** launching the headed test or clicking through the UI.
